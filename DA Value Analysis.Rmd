...
This analysis aims to estimate and visualize the relationship between municipal service costs, infrastructure distribution, and revenue generation across dissemination areas (DAs) within the City of Ottawa. By integrating diverse spatial and tabular datasets, we assess service equity, financial sustainability, and urban form efficiency at a granular level.

* The compiled dataset combines:

* 2021 Census demographic data (population, dwelling counts, land area)

* 2024 Municipal property tax revenue (parcel-level totals aggregated to DA)

* Linear and point infrastructure (e.g., roads, watermains, sewers, fire stations, libraries)

* Parcel tax classification

* Service level indicators

Each DA serves as the unit of analysis, allowing for cross-comparison of per-capita or per-hectare measures of both costs and revenues. Costs of municipal services are approximated using allocation methods adapted from the Hemson Consulting 2013 report on Development Charges Background, including classification by urban/rural typology and service-specific allocation codes.

This dataset supports further evaluation of:

Revenue density ($/ha)

Service cost allocation estimates (e.g., cost of arterial roads per lane-km)

Infrastructure intensity (e.g., road length per capita or per hectare)

Population density and built form indicators (e.g., population vs. households)

This approach facilitates data-driven exploration of which areas contribute more or less in revenue relative to the likely cost of servicing, and can inform planning, budgeting, or advocacy initiatives regarding urban development and financial sustainability.
...
write_csv(regions_budget, "output/regions_budget.csv")
...
```{r}
parcel <- read_csv("QGIS/Parcel/City/Revenue by Parcel.csv",show_col_types = FALSE)
